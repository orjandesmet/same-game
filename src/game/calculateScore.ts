...
export function calculateScore(
  ...
  const normalScore = Math.floor(
    (scoreCard.cellsRemoved ?? 0) * (scoreCard.multiplier ?? 1)
  );
  ...
  const totalCreatureScore = creatureScores.reduce(
    (total, { score }) => total + score,
    0
  );
  _debug?.(
    'Calculated score',
    normalScore + bonus + totalCreatureScore,
    'based on',
    scoreCard
  );
