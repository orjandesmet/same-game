...
function getGroup(board: Board, rowIdx: number, columnIdx: number, color: CellValue, visited: CellKey[] = []): CellKey[] {
  ...
  function removeGroup(board: Board, group: CellKey[]) {
    return board
      ...
      .map((column) => column.toSorted((leftCell, rightCell) => cellUtils.isEmptyCell(leftCell) ? -1 : cellUtils.isEmptyCell(rightCell) ? 1 : -1))
      .toSorted((leftColumn) => cellUtils.isEmptyColumn(leftColumn) ? 1 : -1);
  }
